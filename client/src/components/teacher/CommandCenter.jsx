...
// the PDF → End Session (deletes everything from memory). The Home Front is solo:
// everyone plays the same town's wartime civic coordinator and starts as soon as
// they're approved — no pairing, and one class-wide accuracy group (spec §1, §6).
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">The Home Front · everyone runs the same wartime town · TEKS 7.7E, 7.11A, 7.11B, 7.17B</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">The Home Front · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student takes up the town's story right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-accuracy">
          <div className="trail-stat wide">
            <div className="trail-stat-name"><span aria-hidden="true">🏘️</span> The Home Front — the whole class</div>
      ...
      <footer className="app-footer">7th Grade Texas History · The Home Front · TEKS 7.7E</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [125, 74, 42]; // saddle-brown leather

  doc.setFontSize(16);
  doc.text('The Home Front — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    body: [['The Home Front', String(cls.count), cls.count ? `${cls.average}%` : '—']],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · The Home Front · TEKS 7.7E', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`home_front_${roster.joinCode}_${date}.pdf`);
}
