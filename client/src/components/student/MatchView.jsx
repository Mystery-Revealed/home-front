...
// Solo, so it's always your turn. Alongside the choices, the Town Panel shows
// the home front growing (an airfield → a bomber plant → farms & the POW camp
// → Main Street) and the war years passing — the build-status view that
// replaces a map (spec §1, §3).

import { useEffect, useState } from 'react';
import { emitAck, errorText } from '../../services/socket.js';
import { Art } from '../../services/assets.jsx';
import TownPanel from '../shared/TownPanel.jsx';
import MetersBar from '../shared/MetersBar.jsx';

const homeFrontScore = (m) => (m ? (m.warEffort || 0) + (m.community || 0) + (m.fairness || 0) : 0);
...
export default function MatchView({ state, dispatch }) {
  ...
  const fairnessLow = (match.meters?.fairness ?? 50) <= 15;
  const otherLow = Object.entries(match.meters || {}).find(([k, v]) => k !== 'fairness' && v <= 15);

  return (
    <div className="match">
      <header className="match-header">
        <div className="nation-chip colony">🏘️ <b>The Home Front</b></div>
        <div className="trail-chip" title="Everyone runs the same wartime town">1941–1945</div>
        <div className="hold-chip" title="Your three meters added up (max 300)">
          Home Front Score <b>{homeFrontScore(match.meters)}</b><span className="muted"> / 300</span>
        </div>
        {phase && (
      ...
      <div className="meters-row solo">
        <MetersBar meters={match.meters} meta={meta} title="Your Town" />
      </div>

      {fairnessLow && !feedback && (
        <div className="banner danger" role="alert">
          ⚖️ Your Fairness is running very low. Who shares the work — and the credit — is slipping. Hold the line as long as you can.
        </div>
      )}
      {!fairnessLow && otherLow && !feedback && (
        <div className="banner danger" role="alert">
      ...
      <div className="match-body">
        <section className="action-panel" aria-live="polite">
          {feedback ? (
            ...
            <div className="waiting-panel"><div className="pulse-dot" aria-hidden="true" /><p>On the home front…</p></div>
          )}
        </section>

        <section className="map-panel">
          <TownPanel meters={match.meters} chapterIndex={chapterIndex} />
        </section>
      </div>
    </div>
  );
...
const VERDICT_UI = {
  right: { label: 'The wise, fair call', className: 'right', icon: '✓' },
