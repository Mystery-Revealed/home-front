...
// and inspects the emit instructions it returns. The Home Front is solo with ONE
// class-wide role (a single side, no rival), so these focus on the solo
// lifecycle, one class-wide accuracy group, and — because this game has NO
// early-fail — that every run plays all 12 actions through to match:end.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { PHASES } from '../src/games/homeFront.js';
...
test('the default game is The Home Front', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'home-front');
});
...
test('solo student runs the town and completes with 100% accuracy → A Town That Answered', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  ...
  assert.equal(begin[0].payload.side, SIDE, 'the one side is the town');
  ...
  assert.deepEqual(begin[0].payload.meters, { warEffort: 50, community: 50, fairness: 50 });
  ...
  assert.equal(end[0].payload.you.ending.key, 'top', 'a perfect run is A Town That Answered');
...
test('all-wrong run plays all 12 actions to the end (no early-fail) → Strained Seams, 0%', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
...
test('class accuracy is one class-wide group (the town)', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
